...
const MAX_LAYER_DEVICE_PX: f64 = 12_000.0;
...
// navbar - can stop painting while the DOM still looks correct.
//
// The limit is in DEVICE pixels, so a Retina display hits it at half the CSS
// width an ordinary one does: that is why the artefact showed up on a Mac and
// not elsewhere. Budgeting in device pixels keeps the cap correct on any
// display. offsetWidth is the untransformed layout width, and it tracks
// font-size rather than the viewport, so this holds on a large screen too.
fn name_scale_cap(document: &web_sys::Document, device_pixel_ratio: f64) -> f64 {
    ...
    let ratio = match device_pixel_ratio > 0.0 {
        true => device_pixel_ratio,
        false => 1.0,
    };

    match base_width {
        Some(width) => (MAX_LAYER_DEVICE_PX / (width * ratio)).max(MIN_NAME_SCALE_CAP),
...
fn apply_hero_zoom() {
    ...
    let scale = (1.0 + eased * MAX_NAME_SCALE).min(name_scale_cap(&document, device_pixel_ratio));
